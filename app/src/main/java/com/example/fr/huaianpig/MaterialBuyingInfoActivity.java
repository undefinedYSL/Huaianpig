 ...
 */
public class MaterialBuyingInfoActivity extends Activity {
...
//    private String path = "http://192.168.43.222:9999/myApps";
    ...
    private String[] arr;
    private String[] arr1;
    ArrayList<ItemBeanMaterialUsingInfo> list=null;
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        requestWindowFeature(Window.FEATURE_NO_TITLE);
        setContentView(R.layout.materialbuyinginfo_layout);
        ...
        title_set_bn.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                intent.setClass(MaterialBuyingInfoActivity.this, SystemActivity.class);
                ...
                public void run() {
                    if (spinnerID == 0){
                        mResponse = GetPostUtil.sendPost(
                                path.getIp(), "getfeedbuyininfo@");
                        handler.sendEmptyMessage(0x456);
                    }else if(spinnerID == 1){
                        mResponse = GetPostUtil.sendPost(
                                path.getIp(), "getmedicinebuyininfo@");
                        handler.sendEmptyMessage(0x456);
                    }else if (spinnerID == 2){
                        mResponse = GetPostUtil.sendPost(
                                path.getIp(), "getvaccinebuyininfo@");
                        // 发送消息通知UI线程更新UI组件
                        handler.sendEmptyMessage(0x456);
                    }
                }
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 0x456) {
                ListView listView = (ListView) findViewById(R.id.list_mainmaterial);
                list = new ArrayList<>();
                ...
                if (arr.length>1){
                    for (int i=0;i<arr.length;i++){
                        arr1 = arr[i].split(",");
                        list.add(new ItemBeanMaterialUsingInfo(arr1[0],arr1[1],arr1[3],arr1[6]));
                    }
                    MyBaseAdapterMaterialUsing myBaseAdapter = new MyBaseAdapterMaterialUsing(list,MaterialBuyingInfoActivity.this);
                    listView.setAdapter(myBaseAdapter);
                }else {
                    Toast.makeText(MaterialBuyingInfoActivity.this,"数据库为空",Toast.LENGTH_SHORT).show();
                }
            }
        }
    };
    ...
    protected void dialog()
    {
        Dialog dialog = new AlertDialog.Builder(this).setTitle("智能猪场管理终端").setMessage(
            ...
            public void onClick(DialogInterface dialog, int which) {
                // TODO Auto-generated method stub
                dialog.dismiss();
                MaterialBuyingInfoActivity.this.finish();
            }
